...
sumn = 0;
for K = 1:num_labels
    sumn += ((y == K)'*log(a3(:, K)) + (1- (y == K))'*log(1 - a3(:, K)));
endfor
J = (-1/m)*sumn; % Unregularized cost function

J = J + (lambda/(2*m))*(sum(sumsq(Theta1(:, 2:end))) + sum(sumsq(Theta2(:, 2:end)))); % Regularized cost function
...
for t = 1:m
    ...
    a3 = sigmoid(z3);
    % Calculate deltas
    delta3 = a3*0.0; % Initialize delta3 using a3
    for K = 1:num_labels
        delta3(K) = a3(K) - (y(t) == K);
    endfor
    delta2 = (Theta2'*delta3) .* [1; sigmoidGradient(z2)];
    delta2 = delta2(2:end);
    % Accumulate gradients for this training example
    grad1 += delta2*[1 a1'];
    grad2 += delta3*[1 a2'];
endfor
Theta1_grad = (1/m)*grad1;
Theta2_grad = (1/m)*grad2;
% Regularize Theta_grad
