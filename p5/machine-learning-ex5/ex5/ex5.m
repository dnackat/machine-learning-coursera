...
%% =========== Part 9: Test set error (optional) =============
%  Calculate test set error based on lambda that gave the lowest cross-
%  validation error and the corresponding theta computed by training with
%  that lamda.
%
lambda = 3;
theta = trainLinearReg(X_poly, y, lambda);
testSetError = linearRegCostFunction(X_poly_test, ...
                ytest, theta, 0);

fprintf('\nTest set error for lamda of %i is: %f\n', lambda, testSetError);

fprintf('Program paused. Press enter to continue.\n');
pause;

%% =========== Part 10: Randomly selected examples (optional) =============
...
lambda = 0.01;
num_trials = 50;
[error_train, error_val] = randomTrials(X_poly, y, X_poly_val, yval, ...
                            lambda, num_trials);

plot(1:m, error_train, 1:m, error_val);
title('Learning curve for linear regression using random trials')
legend('Train', 'Cross Validation')
xlabel('Number of training examples')
ylabel('Error')
axis([0 13 0 150])
